...
# rabbit
rabbit_name_1=$(docker compose --project-name demo-stack ps --quiet rabbit-1)
rabbit_name_2=$(docker compose --project-name demo-stack ps --quiet rabbit-2)
rabbit_name_3=$(docker compose --project-name demo-stack ps --quiet rabbit-3)
...
# roach
roach_name=$(docker compose --project-name demo-stack ps --quiet roach-1)

docker exec -it $roach_name sh -c "cockroach init --certs-dir=/certs/node-1 --host=roach-1:26357"
docker exec -i $roach_name sh -c "cockroach sql --certs-dir=/certs/node-1 --host=roach-1:26257" < ../../stack/roach/init.sql
